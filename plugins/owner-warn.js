let handler = async (m, { conn, args, usedPrefix }) => {
    if (args.length > 0) {
        ...
        if (warning < 5) {
            ...
            conn.sendButton(ban, hiasan, '*Kamu di warn oleh moderator, dan sekarang kamu punya ' + (warning + 1) + ' warn. Ingat Jika kamu mendapat warn 5 kali kamu akan otomatis ke banned*', `${imgr + 'Warn'}`, [['⫷ ᴏᴡɴᴇʀ ⫸', usedPrefix + 'owner']],fpay)
        } else if (warning == 5) {
            ...
            conn.sendButton(ban, hiasan, '*Kamu ke banned karena telah mendapatkan 5 kali warning*', `${imgr + 'User Banned'}`, [['⫷ ᴏᴡɴᴇʀ ⫸', usedPrefix + 'owner']],fpay)
        }
